...
jest.mock('aws-sdk', () => {
  return {
    S3: jest.fn(() => ({
      createBucket: jest.fn().mockReturnValue({ promise: () => Promise.resolve() }),
...
describe('isPathyChar', () => {
  test('returns false for invalid file characters', () => {
    ...
    for (const c of invalidFileChars) {
...
describe('isPathyPart', () => {
  ...
  test('returns false when part contains ".", "/" and "\\\\"', () => {
    // Use a literal that is exactly 12 chars: one backslash + "foo/bar.txt"
